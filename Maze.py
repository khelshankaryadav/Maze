...
yaxis = tpx.Scale(r, from_=0, to_=(
    imo.size[1]-1), orient=VERTICAL, style="TScale", command=lambda s: slidery.set('%d' % float(s)))
yaxis.set(1)
yaxis.grid(column=0, row=3, sticky=W+N+S, padx=5)
# Image Dimension Label
dimen = tpx.Label(
    r, text=f"Image Dimnesion: {imo.size[0]} x {imo.size[1]}", style="TLabel")
...
xaxis = tpx.Scale(r, from_=0, to_=(
    imo.size[0]-1), orient=HORIZONTAL, style="TScale", command=lambda s: sliderx.set('%d' % float(s)))
